...
function drawLabels() {
  text("Jan      Feb      Mar      Apr      May     June    July      Aug     Sept      Oct      Nov    Dec", 2*gridOffset, gridOffset);
  for (i = 0; i <= 30; i++) {
    text(i + 1, gridOffset + (rectWidth/2), 1.7*rectHeight + gridOffset + i*rectHeight);
  }
}
...
function drawGrid() {
  noStroke();
  for (i = 0; i <= 365; i++) {
    fill(map(rank[i], 0, 365, 0, numColors)); 
    rect(rectWidth * getMonth(i) + gridOffset, rectHeight * getDay(i)+ gridOffset, rectWidth - 2, rectHeight - 2);
  }
}
